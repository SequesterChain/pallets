...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			log::info!("calculated fees for block {:?}", block_fee_sum);
			...
			if (block_number % T::OnChainUpdateInterval::get()).is_zero() {
				...
				log::info!("pending txn fees queued {:?}", block_fee_sum);
			}
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		fn calculate_fees_for_block() -> BalanceOf<T> {
			...
			for filtered_event in filtered_events {
				match filtered_event {
					<pallet_transaction_payment::Event<T>>::TransactionFeePaid {
						...
						log::info!("found txn fee {:?}", actual_fee);
			...
			log::info!("total fees for block {:?}", curr_block_fee_sum);
		...
		fn update_offchain_storage(block_fee_sum: BalanceOf<T>) {
			...
			let mut lock = StorageLock::<Time>::new(&DB_LOCK);
			{
				...
				match val.get::<BalanceOf<T>>() {
					// initialize value
					Ok(None) => {
						log::info!("value was none, now {:?}", block_fee_sum);
